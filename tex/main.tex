...
In presenting our results, we again report average runtimes and exact solutions and runtimes are available in the lob files in the \texttt{benchmarks} folder in the Github repository and tabular results may be found in the appendix (TODO: REF). We provide graphs which group results by data type and algorithm, so one may compare the performance of each algorithm on one data type as well as the performance of one algorithm on each data type.

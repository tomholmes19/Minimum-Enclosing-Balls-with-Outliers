...
On hyperspherical shell data we observe a similar story to that of uniform ball data, with the relaxation-based heuristic and shrink heuristic giving the same results, with the shrink heuristic running in less time for each parameter. Shenmaier's approximation again is the fastest method for $d$ greater than 70, but like before has strictly worse performance.


\benchmarkfiguresix{uniform_ball_with_outliers}{Uniform Ball with Outliers}
On uniform ball with outliers data, the relaxation-based heuristic is our best performing method, though we observe a larger than usual increase in runtime as the dimension increases, with a runtime of a little over an hour for each instance at $d=80$, so we were unable to solve instances for $d=90$ and $d=100$ within the time constraints. The shrink heuristic as usual gives short runtimes with only marginally worse radii. Recall that the optimal solution for our uniform ball with outliers data will be a radius of 1, which is indeed returned by the relaxation-based heuristic while the shrink heuristic returns radii marginally above 1. Shenmaier's approximation gives far worse than expected solutions, on average around 55\% worse than optimal.

...
\end{algorithm}
A caveat to this approach is that it the value of $M$ will change depending on the initial random point chosen, and can vary significantly. To demonstrate this, we generate a uniform ball data set $\A$ with $n=1000$, $d=10$, $r=1$ (see Section \ref{uniform ball} for further details) which we know has a diameter close to 2. Now, if we run Algorithm \ref{alg:diameter approx} for each $a\in\A$, we 
